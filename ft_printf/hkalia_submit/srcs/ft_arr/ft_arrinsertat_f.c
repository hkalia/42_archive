...
/*   Updated: 2016/11/19 18:40:05 by hkalia           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
						...
						, size_t src_len)
{
	...
	FT_ARR_GRD2(!(tmp = ft_arrnew((*dst)->arr_len - i)), src_cpy, 0);
	j = i;
	while (j < (*dst)->arr_len)
						...
						, size_t src_len)
{
	...
	{
		FT_ARR_GRD2(!(tmp = ft_arrnew(i + src_len)), src_cpy, 0);
	}
	else
		FT_ARR_GRD2(!(tmp = ft_arrnew((*dst)->arr_len + src_len)), src_cpy, 0);
	j = 0;
						...
						, size_t src_len)
{
	...
	FT_ARR_GRD2(!(*dst = ft_arrnew(i + src_len)), src_cpy, 0);
	(*dst)->arr_len = i;
	j = 0;
	while (j < src_len)
