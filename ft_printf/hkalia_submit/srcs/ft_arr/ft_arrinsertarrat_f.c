...
/*   Updated: 2016/11/20 16:53:03 by hkalia           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int8_t	case2(t_arr *dst, size_t i, t_arr *src)
{
	...
	FT_ARR_GRD(!ft_arrinit(&tmp, dst->arr_len - i), src, 0);
	j = i;
	while (j < dst->arr_len)
...
static int8_t	case1(t_arr *dst, size_t i, t_arr *src)
{
	...
	{
		FT_ARR_GRD(!ft_arrinit(&tmp, i + src->arr_len), src, 0);
	}
	else
...
static int8_t	case0(t_arr *dst, size_t i, t_arr *src)
{
	...
	FT_ARR_GRD(!ft_arrinit(dst, i + src->arr_len), src, 0);
	dst->arr_len = i;
	j = 0;
	while (j < src->arr_len)
