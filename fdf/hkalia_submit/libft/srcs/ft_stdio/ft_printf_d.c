...
/*   Updated: 2016/12/19 15:54:17 by hkalia           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int8_t	width_handler_d(t_ft_printf *s)
{
	...
	{
		flg = s->flg_zero == 1 && (s->new.arr[0] == '-' || s->flg_space == 1
									|| s->flg_plus == 1) ? 1 : 0;
		GRD((tmp = (char *)malloc(sizeof(char) * diff)) == 0, -1);
...
int8_t			ft_printf_d(t_ft_printf *s)
{
	...
	if (s->flg_dot == 1 && s->flg_zero == 1)
		s->flg_zero = 0;
	GRD2(dot_handler_d(s) == -1, free(s->new.arr), free(s->ret.arr), -1);
	if (s->new.arr[0] != '-' && (s->flg_plus || s->flg_space))
		GRD(arr_insertat(&s->new, 0, s->flg_plus ? "+" : " ", 1) == -1, -1);
	GRD2(width_handler_d(s) == -1, free(s->new.arr), free(s->ret.arr), -1);
