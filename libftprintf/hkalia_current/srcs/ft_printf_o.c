...
/*   Updated: 2017/01/31 14:56:06 by hkalia           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int8_t	dot_h_o(t_ft_printf *s)
{
	char	*tmp;
	int		diff;
...
uintmax_t		len_mod_h(va_list arg, t_ft_printf *s)
{
	if (s->len_mod == 0)
							...
							, t_ft_printf *s)
{
	...
	if (s->flgs & 0x8 && s->new.ptr[0] != '0')
		GRD2(ft_printf_arr_insert(&s->new, 0, "0") == -1, ft_printf_arr_dtr(&s->new)
			, ft_printf_arr_dtr(ret), -1);
	GRD2(dot_h_o(s) == -1, ft_printf_arr_dtr(&s->new), ft_printf_arr_dtr(ret), -1);
	GRD2(ft_printf_width_h_o(s) == -1, ft_printf_arr_dtr(&s->new)
		, ft_printf_arr_dtr(ret), -1);
