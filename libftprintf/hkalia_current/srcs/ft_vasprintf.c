...
/*   Updated: 2017/01/30 13:59:53 by hkalia           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
							...
							, t_ft_printf *s)
{
	...
	GRD2(ft_printf_arr_appendarr(ret, &s->new) == -1
...
static int		iterator(char **final, const char *fmt, va_list arg)
{
	...
	GRD1((*final = ft_printf_arr_tostr(&ret)) == 0, ft_printf_arr_dtr(&ret), -1);
	return ((int)ret.len);
}
...
int				ft_vasprintf(char **ret, const char *fmt, va_list arg)
{
	...
	*ret = 0;
	if (ft_printf_strchr(fmt, '%') == 0)
		return ((*ret = ft_printf_strdup(fmt))
				? (int)ft_printf_strlen(fmt) : -1);
	va_copy(clone, arg);
